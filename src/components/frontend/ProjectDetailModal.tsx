      ...
      {selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative w-full max-w-3xl bg-white dark:bg-slate-900 border-3 sm:border-4 border-black dark:border-zinc-200 rounded-2xl sm:rounded-3xl shadow-[6px_6px_0px_0px_#000] sm:shadow-[12px_12px_0px_0px_#000] dark:shadow-[6px_6px_0px_0px_#38BDF8] sm:dark:shadow-[12px_12px_0px_0px_#38BDF8] overflow-hidden my-4 sm:my-8 z-10 transition-colors max-h-[90vh] flex flex-col"
          >
            {/* Modal Top Bar */}
            <div className="bg-amber-300 dark:bg-amber-400 border-b-3 sm:border-b-4 border-black p-3 sm:p-4 flex items-center justify-between gap-2.5 sm:gap-4 shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                <span className="bg-black text-white px-2 sm:px-2.5 py-0.5 rounded text-[10px] sm:text-xs font-black shrink-0">
                  {getProjectCategory(selectedProject)}
                </span>
                <h3 className="font-black text-sm sm:text-lg text-black truncate">
              ...
              <motion.button
                whileTap={{ scale: 0.9 }}
                onClick={() => setSelectedProject(null)}
                className="w-8 h-8 sm:w-9 sm:h-9 bg-white border-2 border-black rounded-xl flex items-center justify-center font-black shadow-[2px_2px_0px_0px_#000] hover:bg-rose-300 transition-colors shrink-0"
              >
            ...
            <div className="overflow-y-auto flex-1">
              {/* Modal Image Display */}
              <div className="relative h-48 sm:h-64 md:h-80 bg-zinc-100 dark:bg-slate-950 border-b-3 sm:border-b-4 border-black dark:border-zinc-300 overflow-hidden">
              ...
              <div className="p-4 sm:p-6 md:p-8 flex flex-col gap-4 sm:gap-5">
                ...
                {getProjectDescription(selectedProject) && (
                  <div>
                    ...
                    <p className="text-xs sm:text-sm font-bold text-zinc-700 dark:text-zinc-200 leading-relaxed whitespace-pre-line">
                      {getProjectDescription(selectedProject)}
                    </p>
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2.5 sm:gap-3 pt-2 sm:pt-4">
                  {selectedProject.demoUrl && (
                    <motion.a
                      ...
                      whileTap={{ scale: 0.97 }}
                      className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 bg-black text-yellow-300 border-2 border-black py-2.5 sm:py-3 rounded-xl text-xs font-black shadow-[2px_2px_0px_0px_#000] sm:shadow-[3px_3px_0px_0px_#FFE4E6] hover:bg-zinc-800 transition-colors min-h-[40px]"
                    >
                  ...
                  {selectedProject.githubUrl && (
                    <motion.a
                      ...
                      rel="noopener noreferrer"
                      whileTap={{ scale: 0.97 }}
                      className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 bg-emerald-300 text-black border-2 border-black py-2.5 sm:py-3 rounded-xl text-xs font-black shadow-[2px_2px_0px_0px_#000] sm:shadow-[3px_3px_0px_0px_#000] hover:bg-emerald-400 transition-colors min-h-[40px]"
                    >
                      <span>{t.viewGithub}</span>
                      <Github className="w-3.5 h-3.5 sm:w-4 sm:h-4 stroke-[2.5]" />
                    </motion.a>
                  )}
                </div>
              </div>
